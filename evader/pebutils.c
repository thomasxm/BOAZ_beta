...
ULONG LdrHashEntry(UNICODE_STRING UniName, BOOL XorHash) {
	ULONG ulRes = 0;
	RTLHASHUNICODESTRING pRtlHashUnicodeString = (RTLHASHUNICODESTRING)GetFunctionAddress(IsModulePresent(L"ntdll.dll"), (char*)"RtlHashUnicodeString");
...
{
	RTLRBINSERTNODEEX pRtlRbInsertNodeEx = (RTLRBINSERTNODEEX)GetFunctionAddress(IsModulePresent(L"ntdll.dll"), (char*)"RtlRbInsertNodeEx");
...
{
	MULTIBYTETOWIDECHAR pMultiByteToWideChar = (MULTIBYTETOWIDECHAR)GetFunctionAddress(IsModulePresent(L"kernel32.dll"), (char*)"MultiByteToWideChar");
...
)
{
	...
	{
		ok = _LocalLdrGetProcedureAddress(
			(HMODULE)kernel32_addr, //kernel32_addr,
			ProcName,
			Ordinal,
			FunctionAddress
		);
...
)
{
	...
	for (int i = 0; i < pNtHeaders->FileHeader.NumberOfSections; i++)
	{
		...
		if (my_strncmp(".text", (const char*)pSecHeader->Name, 6))
		{
			...
			if (found)
			{
				...
				if (startValidSection > FunctionPtr || FunctionPtr > endValidSection)
				{
					...
					if (lib_length != 0)
					{
						...
						// my_strncpy(libname + lib_length, (const char*)".dll", 5);
...
BOOL AddModuleToPEB(
	...
	ULONG_PTR ModuleBase // base address of the module
...
)
{
	...
	HEAPALLOC pHeapAlloc = (HEAPALLOC)GetFunctionAddress(IsModulePresent(L"Kernel32.dll"), (char*)"HeapAlloc");
	RTLINITUNICODESTRING pRtlInitUnicodeString = (RTLINITUNICODESTRING)GetFunctionAddress(IsModulePresent(L"ntdll.dll"), (char*)"RtlInitUnicodeString");
	NTQUERYSYSTEMTIME pNtQuerySystemTime = (NTQUERYSYSTEMTIME)GetFunctionAddress(IsModulePresent(L"ntdll.dll"), (char*)"NtQuerySystemTime");

	pNtHeaders = RVA(
		PIMAGE_NT_HEADERS, 
		pbDllData, // pbDllData is the base address of the module, for instance, dll_base or fileBase.
		((PIMAGE_DOS_HEADER)pbDllData)->e_lfanew
	);

	// convert the names to unicode
	pRtlInitUnicodeString(
		&FullDllName, 
		LocalDLLName // LocalDLLName is the full path to the module
	);
	...
	AddBaseAddressEntry(
		pLdrEntry,
		(PVOID)ModuleBase // ModuleBase is the base address of the module, (ULONG_PTR);
	);
	...
	pLdrEntry->DllBase               = (PVOID)ModuleBase;
	...
	pLdrEntry->Flags                 = LDRP_IMAGE_DLL | LDRP_ENTRY_INSERTED | LDRP_ENTRY_PROCESSED | LDRP_LOAD_NOTIFICATIONS_SENT; //If we set LDRP_PROCESS_ATTACH_CALLED, dll entry point will be triggered by ResumeThread
	...
	// );
	pLdrEntry->EntryPoint = (PLDR_INIT_ROUTINE)RVA(PVOID, ModuleBase, pNtHeaders->OptionalHeader.AddressOfEntryPoint);
